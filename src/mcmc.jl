...
function mcmc_risk_splines(data::StepStressData,base_haz_splines::Splines,risk_splines::Splines,n_mcmc::Int,steps::StepSize,s_map::Array{Int,2},init_vals)
    fail_indic = sum(data.delta_i[2:(end-1),:],dims=2)
    ...
    main_risk = [sum_risk(j,risk_splines.M,beta,data.in_risk_idx,s_map) for j in 2:(J-1)]
    off_risk = [sum_risk(j,risk_splines.M,beta,data.in_risk_idx,s_map) for j in 2:(J-1)]
...
function bulk_mcmc_baseline_splines(data::StepStressData,splines::Splines,
    ...
    #end
    if multithread
        results = _par_bulk_mcmc_baseline_splines(data,splines,n_mcmc,steps,init_vals,
    n_burn,lag,max_arr_len)
    else
        results = _bulk_mcmc_baseline_splines(data,splines,n_mcmc,steps,init_vals,
    n_burn,lag,max_arr_len)
    end
